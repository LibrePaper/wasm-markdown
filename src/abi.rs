...
/// Tells the compiler what day it is, for `datetime.today()`. Markdown has no
/// clock and no dates, so this module does nothing with it and exports it only
/// so that one loader drives every renderer without first asking which it has.
#[no_mangle]
